...
class isys_notification_maintenance_period extends isys_notification
{
    ...
    protected function handle_notification($p_notification)
    {
        ...
        if ($l_num == 0) {
            $this->m_log->debug('There are no objects left to report.');
        ...
        foreach ($l_objects AS $l_object) {
            ...
            $l_end_of_period = $l_contract_dao->calculate_maintenanceperiod($l_object['isys_cats_contract_list__start_date'],
                $l_object['isys_cats_contract_list__maintenance_period'], $l_object['isys_cats_contract_list__maintenance_period_unit__id']);
